...
function visSlides(n) {
	...
	// Hvis n (her 3) > 2 så sæt slideNummer = 1, således at billede 1 bliver vist (se consollen)
	if (n > slides.length) {
		console.log("N = " + n); // N = 3 da n bliver 2(slideNummer) + 1(n) = 3
	...
	// Hvis n (her 0) < 1 så sæt slideNummer = 2 således at billede 2 bliver vist
	if (n < 1) {
		console.log("N = " + n); // N = 0 da n bliver 2(slideNummer) + 1(n) = 3
		console.log("Hvor mange img'er der = " + slides.length); // Der er 2 diver med classen billede_slideshow

		slideNummer = slides.length // Sæt slideNummer = 2 dvs vis billede 2
	}
	...
	slides[slideNummer - 1].style.display = "block"; // det billede hvor i = 0 får display block, således at man ikke kan se billedet
